class Solution:
    def countSubIslands(self, grid1: List[List[int]], grid2: List[List[int]]) -> int:
        ...
        def bfs(grid, start_r, start_c, idx: int = -1, id2set: dict = None) -> set:
            cell_set = set()
            queue = deque()
        ...
        for r in range(nrows):
            for c in range(ncols):
                if grid2[r][c] == LAND:
                    island_2 = bfs(grid2, r, c)
                    # check island_2's idx on grid1 using (r, c)
                    # check is subset (subisland)
                    island_idx = grid1[r][c]
                    if (island_idx in id2set) and island_2.issubset(id2set[island_idx]):
                        count += 1
